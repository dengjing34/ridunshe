...
$(function(){
    ...
    Stip('nav_1').show({content:'一级菜单位置', kind:'error', p:'top'});//time:1000
    Stip('pageSubheader').show({content:'二级菜单位置', kind:'error', p:'top'});//time:1000
    Stip('crumb').show({content:'操作按钮位置', kind:'correct', p:'top'});//time:1000
});
</script>
<style>
...
foreach ($access as $first) {
    ...
    $accessHtml .= "<dt><input type=\"checkbox\" name=\"access[]\" id=\"access_{$first['id']}\" value=\"{$first['id']}\" /> <label for=\"access_{$first['id']}\">{$first['cname']}</label></dt>\n";
    if(isset ($first['children'])){
        foreach ($first['children'] as $second) {
            $accessHtml .= "<dd><input type=\"checkbox\" name=\"access[]\" id=\"access_{$second['id']}\" value=\"{$second['id']}\" /> <label for=\"access_{$second['id']}\">{$second['cname']}</label>";
            if(isset ($second['children'])){
                $accessHtml .= "<ul>\n";
                foreach ($second['children'] as $third) {
                    $accessHtml .= "<li><input type=\"checkbox\" name=\"access[]\" id=\"access_{$third['id']}\" value=\"{$third['id']}\" /> <label for=\"access_{$third['id']}\">{$third['cname']}</label></li>";
                }
                $accessHtml .= "</ul>\n";
            }
            $accessHtml .= "</dd>\n";
