...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase
{
    ...
    public function sendResponse()
    {
		...
		$sFileName = date("Ymd") . "_" . date("His") . '_'. rand('1111', '9999') . "." . $sExtension; 	
		//按上传时间重命名文件结束	        
		...
		$sServerDir = $monthDir;
		$oRegistry->set("FileUpload_url", $this->_currentFolder->getUrl().date('Y').'/'.date('m').'/');//这一个要传递给ErrorHandler的FileUpload.php文件，用以显示上传后的地址
		//创建日期文件夹结束
        while (true)
        {
